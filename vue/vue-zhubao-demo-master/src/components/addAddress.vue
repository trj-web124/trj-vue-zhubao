  ...
  <div id="addAddress">
  	<yd-cell-group>
        <yd-cell-item>
            ...
            <yd-input slot="right" v-model="receiveName" required :showClearIcon="false" placeholder="请输入您的姓名"></yd-input>
        </yd-cell-item>
        <yd-cell-item>
            <span slot="left">手机号：</span>
            <yd-input slot="right" v-model="receivePhone" regex="mobile" placeholder="请输入手机号码"></yd-input>
        </yd-cell-item>
        <yd-button size="large" type="primary" style="background-color: #FC83AB;" @click.native="openCitySelect">{{receiveDistr ? `${receiveProv} ${receiveCity}  ${receiveDistr}` : '选择地址'}}</yd-button>
        <yd-cell-item>
            <yd-textarea slot="right" placeholder="请输入您的详细地址" maxlength="100" :change="setReceiveAddr" v-model="receiveAddr"></yd-textarea>
        </yd-cell-item>
    </yd-cell-group>
...
<script>
import District from '../api/gov_province_city_area_id.js';
...
export default {
  ...
  methods: {
  	...
  	setReceiveAddr () {
  		// 官方文档说要用callback来绑定回调函数，但是这里会报错，用旧版本的change绑定却可以
  		console.log(this.receiveAddr)
  	},
  	result1(ret) {
    ...
    save () {
    	...
    	console.log(this.receiveProv)
    	console.log(this.receiveCity)
    	console.log(this.receiveDistr)
    	console.log(this.receiveAddr)
    	if (!this.receiveName) {
    		this.$toast({
    			text: '请输入您的姓名'
